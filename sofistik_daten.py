...
class CMAT_USER(Structure):        # 1/NR:10??  Information on user input
   _fields_ = [
         ...
         ('m_bit', c_int * 32)     #        User input for 001/NR:1,2,3,4,7,8,9,14,15,16
      ]
cmat_user = CMAT_USER()
...
class CMAT_LAY(Structure):         # 1/NR:8  MaterialLayerStructure
   _fields_ = [
         ...
         ('m_beta', c_float)       # [1200] Angle of material (difference to local x axis of element)
      ]
cmat_lay = CMAT_LAY()
...
class CMAT_CRE(Structure):         # 1/NR:90  ExplicitCreepCurve
   _fields_ = [
         ('m_id', c_int),          #        Identification = 90
         ('m_type', c_int),        #        type of material value = MAT_SPE_TYPE_EIGC_EXP_CREEP_CURVE = 10
...
class CMAT_SHR(Structure):         # 1/NR:90  ExplicitShrinkageCurve
   _fields_ = [
         ('m_id', c_int),          #        Identification = 90
         ('m_type', c_int),        #        type of material value = MAT_SPE_TYPE_EIGS_EXP_SHRINK_CURVE = 1
         ('m_title', c_int),       #        title of the curve
         ('m_rval', c_float * 16 * 2)  # [1081] real material values (time[d],value pairs),
      ]
cmat_shr = CMAT_SHR()

class CMAT_GWP(Structure):         # 1/NR:91  Global Warming Potential (CO2 Equivalents)
   _fields_ = [
         ...
         ('m_phase',  c_int * 9)   #        life cycle module may be user defined, depending on the design 
      ]
cmat_gwp = CMAT_GWP()
...
class CSECT_REC(Structure):        # 9/NR:10  Standard Rectangular, T-Beam     (SREC)
   _fields_ = [
         ...
         ('m_bo', c_float),        # [1011] effective width of plate
         ('m_daso', c_float),      # [1023] reinforcement Diameter up
         ('m_dasu', c_float),      # [1023] reinforcement Diameter down
         ...
         ('m_phib', c_float),      # [1200] inclination of transverse reinforcements
         ...
         ('m_dass', c_float),      # [1023] reinforcement diameter lateral
         ...
         ('m_bdl', c_float)        # [1011] total width of plate for additional dead weight
      ]
csect_rec = CSECT_REC()

class CSECT_ANN(Structure):        # 9/NR:11  Standard Circle, Annular (SCIT)
   _fields_ = [
         ...
         ('m_daso', c_float),      # [1023] reinforcement diameter
         ('m_amax', c_float),      # [1011] maximum distance of single reinforcements
         ('m_asbm', c_float),      # [1021] Minimum shear reinforcement
         ('m_hmax', c_float),      # [1011] maximum mesh size for FE/fibre analysis
         ('m_fkit', c_float),      #        <0 = factor for torsional inertia >0 = absolut value of torsion
         ('m_fkay', c_float),      #        <0 = factor for shear deformation area >0 = absolut value of sh
         ('m_fkaz', c_float)       #        <0 = factor for shear deformation area >0 = absolut value of sh
...
class CSECT_TUB(Structure):        # 9/NR:11  Standard Tube            (TUBE)
   _fields_ = [
         ('m_id', c_int),          #        Identification = 11
...
class CSECT_CAB(Structure):        # 9/NR:11  Cable            (CABL)
   _fields_ = [
         ('m_id', c_int),          #        Identification = 11
...
class CSECT_PRO(Structure):        # 9/NR:12  Standard rolled steel shapes (PROF)
   _fields_ = [
...
class CSECT_SHW(Structure):        # 9/NR:13  Standard shear wall (SHRW)
   _fields_ = [
         ...
         ('m_a', c_float),         # [1011] distance of single reinforcements     !Abstand Bewehrungseisen
         ('m_d', c_float),         # [1023] diameter of base reinforcement        !Durchmesser Grundbewehru
         ('m_as', c_float),        # [1020] total area base reinforcement
         ('m_lzp', c_float),       # [1011] length of edge reinforcement Z+
         ('m_azp', c_int),         #        number of bars Z+ (one side)          !Anzahl Bewehrungseisen Z
         ('m_dzp', c_float),       # [1023] diameter of edge reinforcement Z+     !Durchmesser Bewehrung Z+
         ...
         ('m_azn', c_int),         #        number of bars Z- (one side)          !Anzahl Bewehrungseisen Z
         ('m_dzn', c_float),       # [1023] diameter of edge reinforcement Z-     !Durchmesser Bewehrung Z-
...
class CSECT_EDB(Structure):        # 9/NR:14  Standard Edge Beam  (EDGB)
   _fields_ = [
         ('m_id', c_int),          #        Identification = 14
         ('m_iq', c_int),          #        Identification of section
         ('m_ir', c_int),          #        Identification of reinforcement
         ('m_h', c_float),         # [1011] total height
         ('m_b', c_float),         # [1011] width (of web)
         ...
         ('m_dasu', c_float),      # [1023] reinforcement diameter down
         ('m_fkit', c_float),      #        <0 = factor for torsional inertia >0 = absolut value of torsion
         ('m_fkay', c_float),      #        <0 = factor for shear deformation area y>0 = absolut value of s
         ('m_fkaz', c_float),      #        <0 = factor for shear deformation area z>0 = absolut value of s
         ('m_ayz', c_float),       # [1012] RWS = default for shear deformation area yzotherwise = value of
         ('m_ysmp', c_float),      # [1011] explicit location y of shear center
         ('m_zsmp', c_float),      # [1011] explicit location z of shear center
         ('m_ys', c_float),        # [1011] explicit location y of reference point
         ('m_zs', c_float),        # [1011] explicit location z of reference point
         ('m_phib', c_float),      # [1200] inclination of transverse reinforcements
         ('m_beff', c_float),      # [1011] width of equivalent hollow section
         ('m_asbm', c_float),      #        Minimum shear reinforcement
         ('m_a', c_float),         # [1011] spacing of reinforcements
         ('m_naso', c_float),      #        number of reinforcement points up
         ('m_nasu', c_float),      #        number of reinforcement points down
         ('m_ss', c_float),        # [1024] Boundary distance of lateral reinforcement
         ('m_dass', c_float),      # [1023] reinforcement diameter lateral
         ('m_hmax', c_float),      # [1011] maximum mesh size for FE/fibre analysis
         ('m_betc', c_float),      #        friction coefficient
         ('m_mue', c_float),       #        friction in joint
         ('m_bdl', c_float),       # [1011] total width of plate for additional dead weight
         ('m_cutz', c_float)       # [1011] Distance z from the top for the shear cut for web
      ]
csect_edb = CSECT_EDB()
...
class CSECT_SPT(Structure):        # 9/NR:100  SectionStressPoint (SPT/QSP)
   _fields_ = [
         ...
         ('m_rt', c_float),        # [1011] Radius of circular arc ; first radius of ellipse (:101) ; effec
         ...
         ('m_rt2', c_float),       # [1011] second radius of ellipse   (:101) ; unused (:100)
...
class CSECT_PAN(Structure):        # 9/NR:103  SectionPanel (PLAT,WALL)
   _fields_ = [
         ...
         ('m_subid', c_int)        #        sub-Id for split elements
      ]
csect_pan = CSECT_PAN()
...
class CLC_CTRL(Structure):         # 12/LC:?  Information on loadcase LC
   _fields_ = [
         ...
         ('m_ityp', c_int),        #        EITHER: action type (14/ID with ID = ITYP)
         ...
         ('m_name', c_int * 7),    #        Generating source
         ('m_access', c_int),      #        Access number
...
class CLC_SUPE(Structure):         # 12/LC:2  Superposition load case
   _fields_ = [
         ...
         ('m_vtrn', c_float),      # [1201] for RTYP=LC_SUPE_RTYP_MLT_ENV: train velocity
...
class CLC_EIGE(Structure):         # 12/LC:4  Eigenmode load case
   _fields_ = [
         ...
         ('m_dir', c_int),         #        requested dof-direction for max. abs. value mode scaling
         ('m_plc', c_int),         #        Primary load case
         ('m_omega', c_float),     # [1206] circular frequency omega = 2*PI*f [rad/sec]
         ('m_modd', c_float),      # [1299] modal damping ratio [-]
         ('m_tgtv', c_float),      #        for LC_EIGE_ITYP_MXA and LC_EIGE_ITYP_MXN: target value used fo
         ('m_val4', c_float),      #        unused
...
class CLC_COPY(Structure):         # 12/LC:012  Loads from other loadcases
   _fields_ = [
         ...
         ('m_nref', c_int * 8),    #        Load train application control (see cdblfta.f90)
...
class CGLC_GUID(Structure):        # 12/LC:199  Unique identifier of loading member
   _fields_ = [
         ...
         ('m_orig', c_int),        #        Origin of load
         ('m_id1', c_int),         #        reserved
         ('m_id2', c_int),         #        reserved
         ('m_idbim', c_int * 36),  #        256 Bit GUId Identifier of following element
         ('m_tag',  c_int * 17)    #        Load tag: Loads can be taged with the GUID record
      ]
cglc_guid = CGLC_GUID()
...
class CACT_TASK(Structure):        # 14/0  Current superposition task commands
   _fields_ = [
         ...
         ('m_lfm', c_int * 2),     #        base of generated load case numbers
...
class CACC_TASK_SCOM(Structure):   #        index,factor and name of items
   _fields_ = [
         ('m_ind', c_int),
         ('m_fak', c_float),
         ('m_bez', c_int)
      ]
cacc_task_scom = CACC_TASK_SCOM()

class CACC_TASK(Structure):        # 14/-1  Accumulated superposition task commands
   _fields_ = [
         ('m_ivers', c_int),       #        < 9999 = old data format (ACT_RULE)
         ('m_num', c_int),         #        number of superposition command
         ('m_act', c_int),         #        number of groups of actions at 14/act
         ...
         ('m_x', c_float),         # [1001] X-ordinate for beams (see INC!)
         ('m_comb', c_int),        #        combination method for items given at SCOM
         ('m_scom',  CACC_TASK_SCOM * 9), #        index,factor and name of items
         ('m_txt',  c_int * 33),   #        Name of superposition
         ('m_uform', c_int * 128)  #        User defined target function (index == 0!)
      ]
cacc_task = CACC_TASK()
...
class CPERF_CAT(Structure):        # 16/0:0  Definition of performance categories (=sets of performance crit
   _fields_ = [
         ('m_id', c_int),          #        record identifier
         ('m_etyp', c_int),        #        element type, for which the criteria applies
         ('m_cat', c_int),         #        performance category identifier, arbitrary 4-character string
         ('m_unused', c_int * 3),  #        unused
         ('m_title',  c_int * 17)  #        Designation of category
      ]
cperf_cat = CPERF_CAT()

class CPERF_CRT(Structure):        # 16/0:1  Definition of performance criterion
   _fields_ = [
         ('m_id', c_int),          #        record identifier
         ('m_etyp', c_int),        #        element type, record identifier PERF_CRT_ETYP_LNK = 173
         ('m_setno', c_int),       #        set number, unused = 0
         ('m_cat', c_int),         #        performance category, cf. PERF_CAT
         ('m_crt', c_int),         #        short label of criterion (4-character string), e.g., 'OLE'
         ('m_unused', c_int),      #        unused
         ('m_pidx', c_float),      #        performance index (ascending magnitude denotes increasing sever
         ('m_title',  c_int * 17)  #        Designation of criterion
      ]
cperf_crt = CPERF_CRT()

class CPLIM_LNK(Structure):        # 16/0:173  Definition of link/spring/hinge element performance thresholds 
   _fields_ = [
         ('m_etyp', c_int),        #        element type, record identifier PERF_CRT_ETYP_LNK = 173
         ('m_wlno', c_int),        #        work law number Rec: 173/NR:+  SPRI_WL
         ('m_cat', c_int),         #        performance category, cf. PERF_CAT
         ('m_crt', c_int),         #        short label of criterion (4-character string), cf. PERF_CRT
         ('m_mno', c_int),         #        material number, for which provided threshold strains strains a
         ('m_lstyp', c_int),       #        type of limit:
         ('m_lsc', c_float),       # [1081] Compressive limit strain (< 0.0)
         ('m_lst', c_float)        # [1081] Tensile limit strain (> 0.0)
      ]
cplim_lnk = CPLIM_LNK()

class CPERF_RES(Structure):        # 16/LC  Performance status of elements
   _fields_ = [
         ('m_etyp', c_int),        #        element type, e.g., PERF_CRT_ETYP_LNK
         ('m_cat', c_int),         #        performance category
         ('m_crtmx', c_int),       #        max performance criterion met or exceeded by current system sta
         ('m_limtyp', c_int),      #        type of limit, tension/ compression
         ('m_mno', c_int),         #        material number, for which the strain was met or exceeded
         ('m_wlno', c_int),        #        work law number Rec: 173/NR:+  SPRI_WL
         ('m_rtyp', c_int),        #        type of hinge reaction (see 173/NR:+  SPRI_WL)
         ('m_enr', c_int),         #        element reference (number)
         ('m_ref', c_int),         #        additional reference info
         ('m_ival9', c_int),       #        unused
         ('m_pidxmx', c_float)     #        max performance index
      ]
cperf_res = CPERF_RES()
...
class CMTOT_CTL(Structure):        # 17/00  Control of total mass calculation
   _fields_ = [
         ...
         ('m_rfbasic', c_int),     #        bit pattern of used basic reinforcements
...
class CMTOT(Structure):            # 17/01:-1  Total mass
   _fields_ = [
         ...
         ('m_rfbend', c_float),    # [1180] Contained total mass of bending reinforcements
         ('m_rfshear', c_float),   # [1180] Contained total mass of shear reinforcements
...
class CMTOT_MAT(Structure):        # 17/02  Masses per material
   _fields_ = [
         ...
         ('m_rfbend', c_float),    # [1180] Additional total mass of bending reinforcements
         ('m_rfshear', c_float),   # [1180] Additional total mass of shear reinforcements
...
class CMTOT_DSG(Structure):        # 17/14  Masses of design elements per secondary group
   _fields_ = [
         ('m_no', c_int),          #        Group Id
         ('m_int1', c_int),        #        unused
         ('m_int2', c_int),        #        unused
...
class CMTOT_DSC(Structure):        # 17/15  Masses of design elements per cross section
   _fields_ = [
         ('m_no', c_int),          #        Section number
         ('m_int1', c_int),        #        unused
         ('m_int2', c_int),        #        unused
...
class CMTOT_DTP(Structure):        # 17/16  Masses of design elements per structural member
   _fields_ = [
...
class CMLCA_TOT(Structure):        # 17/101:-1  Total mass of of life cycle assessment
   _fields_ = [
         ('m_id', c_int),          #        Identification = -1
...
class CMLCA(Structure):            # 17/101:-1  Total masses of life cycle assessment per phase
   _fields_ = [
         ('m_id', c_int),          #        Identification = -1
...
class CMLCA_MA0(Structure):        # 17/102:Z+  Total masses of life cycle assessment per material
   _fields_ = [
         ('m_no', c_int),          #        Material number
         ('m_id1', c_int),         #        Identification = -1
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_MAT(Structure):        # 17/102:Z+  Masses of life cycle assessment per material and phase
   _fields_ = [
         ('m_no', c_int),          #        Material number
         ('m_id1', c_int),         #        Identification = 0
         ('m_typlca', c_int),      #        type of life cycle assessment
         ...
         ('m_volume', c_float),    # [1008] Volume
         ('m_massmat', c_float),   # [1180] Mass material
         ('m_dimvol', c_float),    # [1240] Dimension value of CO2 equivalent per volume
         ('m_dimmass', c_float),   # [1239] Dimension value of CO2 equivalent per mass
         ('m_phase',  c_int * 9)   #        Life cycle module
      ]
cmlca_mat = CMLCA_MAT()

class CMLCA_GR0(Structure):        # 17/103:Z+  Total masses of Life cycle assessment per group
   _fields_ = [
         ('m_no', c_int),          #        Group number
         ('m_id1', c_int),         #        Identification = -1
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_GRP(Structure):        # 17/103:Z+  Masses of Life cycle assessment per group and phase
   _fields_ = [
         ('m_no', c_int),          #        Group number
         ('m_id1', c_int),         #        Identification = 0
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_SG0(Structure):        # 17/104:chr  Total masses of Life cycle assessment per secondary group
   _fields_ = [
         ('m_no', c_int),          #        Group id
         ('m_id1', c_int),         #        Identification = -1
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_SGR(Structure):        # 17/104:chr  Masses of Life cycle assessment per secondary group and phase
   _fields_ = [
         ('m_no', c_int),          #        Group id
         ('m_id1', c_int),         #        Identification = 0
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_DS0(Structure):        # 17/114:chr  Total masses of Life cycle assessment of design elements per se
   _fields_ = [
         ('m_no', c_int),          #        Group id
         ('m_id1', c_int),         #        Identification = -1
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_DSG(Structure):        # 17/114:chr  Masses of Life cycle assessment of design elements per secondar
   _fields_ = [
         ('m_no', c_int),          #        Group id
         ('m_id1', c_int),         #        Identification = 0
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_DT0(Structure):        # 17/116:Z+  Masses of Life cycle assessment of design elements per structur
   _fields_ = [
         ('m_id', c_int),          #        Structural member id
         ('m_id1', c_int),         #        Identification = -1
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CMLCA_DTP(Structure):        # 17/116:Z+  Masses of Life cycle assessment of design elements per structur
   _fields_ = [
         ('m_id', c_int),          #        Structural member id
         ('m_id1', c_int),         #        Identification = 0
         ('m_typlca', c_int),      #        type of life cycle assessment
...
class CGLN_MAT0(Structure):        # 31/0:0  Header of heritage of Structural Lines
   _fields_ = [
         ('m_id0', c_int),         #        #0       header record
         ('m_ibt', c_int),         #        reserved bitpattern of properties
         ('m_iln', c_int),         #        number of following master-line entries
         ('m_ival', c_int * 3)     #        reserved
      ]
cgln_mat0 = CGLN_MAT0()
...
class CGLN_MATC(Structure):        # 31/0:+  Heritage of Structural Lines
   _fields_ = [
         ...
         ('m_ibt', c_int),         #        bitpattern of properties
         ('m_ival', c_int * 4),    #        reserved
         ('m_ids', c_int * 9999)   #        sequence of sub-lines
      ]
cgln_matc = CGLN_MATC()

class CGAR_MAT0(Structure):        # 32/0:0  Header of heritage of Structural Areas
   _fields_ = [
         ('m_id0', c_int),         #        #0       header record
         ('m_ibt', c_int),         #        reserved bitpattern of properties
         ('m_iar', c_int),         #        number of following master-area entries
         ('m_ival', c_int * 3)     #        reserved
      ]
cgar_mat0 = CGAR_MAT0()
...
class CGAR_MATC(Structure):        # 32/0:+  Heritage of Structural Areas
   _fields_ = [
         ...
         ('m_ibt', c_int),         #        bitpattern of properties
         ('m_ival', c_int * 4),    #        reserved
         ('m_ids', c_int * 9999)   #        sequence of sub-areas
      ]
cgar_matc = CGAR_MATC()
...
class CSMB_ATTR(Structure):        # 39/NR:997  Attributes of structural member
   _fields_ = [
         ...
         ('m_i1', c_int),          #        reserved
         ('m_i2', c_int),          #        reserved
         ('m_as1', c_int),         #        identifier of grid axis x-x at start of member
         ('m_as2', c_int),         #        identifier of grid axis y-y at start of member
         ('m_ae1', c_int),         #        optional identifier of grid axis x-x at end of member
         ('m_ae2', c_int),         #        optional identifier of grid axis y-y at end of member
...
class CSPT_GUID(Structure):        # 30/NR:999  Unique identifier of structural point
   _fields_ = [
         ('m_id', c_int),          #        identifier 999
         ('m_idbim', c_int * 36)   #        Unique identifier of the structural element
      ]
...
class CSLN_GUID(Structure):        # 31/NR:999  Unique identifier of structural line
   _fields_ = [
         ('m_id', c_int),          #        identifier 999
         ('m_idbim', c_int * 36)   #        Unique identifier of the structural element
      ]
...
class CSAR_GUID(Structure):        # 32/NR:999  Unique identifier of structural area
   _fields_ = [
         ('m_id', c_int),          #        identifier 999
         ('m_idbim', c_int * 36)   #        Unique identifier of the structural element
      ]
...
class CTOWR_GUI(Structure):        # 34/ID:999  Unique identifier of tower
   _fields_ = [
         ('m_id', c_int),          #        identifier 999
         ('m_idbim', c_int * 36)   #        Unique identifier of the structural element
      ]
...
class CSGRP_PRO(Structure):        # 37/ID:3  Optional Additional Properties
   _fields_ = [
         ('m_id', c_int),          #        identification of data (3)
         ('m_dgrp', c_int),        #        Secondary Group is a design group for member types:
         ('m_nref', c_int),        #        Reference object
...
class CSGRP_LIS(Structure):        # 37/ID:0???  Calculated Selective Element List
   _fields_ = [
         ('m_id', c_int),          #        identifier of element to be selected (positive) or to be desele
...
class CTGEOGUID(Structure):        # 41/NR:999  Unique identifier of TGEO (aka tendon spline or duct curve)
   _fields_ = [
         ...
         ('m_guid', c_int * 36)    #        1152 Bit GUId Identifier
      ]
ctgeoguid = CTGEOGUID()
...
class CSLVL_DAT(Structure):        # 87/0:+  Storey Level Data
   _fields_ = [
         ...
         ('m_nref', c_int * 2),    #        Node numbers of reference nodes on storey level
         ('m_idtowr', c_int),      #        Tower ID associated with the storey level
...
class CBEAM_FTR(Structure):        # 112/LC:Z!  Beam forces without plate components
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_x', c_float),         # [1001] distance from start
         ('m_n', c_float),         # [1101] normal force
...
class CDSLN_FTR(Structure):        # 122/LC:Z!  Design element forces without plate components
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_x', c_float),         # [1001] distance from start
         ('m_n', c_float),         # [1101] normal force
...
class CBEAM_STI(Structure):        # 103/LC  Stiffness of beams
   _fields_ = [
         ...
         ('m_erf_min', c_float),   # [1081] minimum strain reinforcement
         ('m_erf_max', c_float),   # [1081] maximum strain reinforcement
...
class CBEAM_CRF(Structure):        # 104/LC:Z!  resulting forces of redistribution
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_id', c_int),          #        identifier 0
         ('m_x', c_float),         # [1001] distance from start
...
class CDSLN_CRF(Structure):        # 124/LC:Z!  resulting forces of redistribution
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_id', c_int),          #        identifier 0
         ('m_x', c_float),         # [1001] distance from start
...
class CBSCT_CRF(Structure):        # 144/LC:Z!  resulting forces of redistribution
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_id', c_int),          #        identifier 0
         ('m_x', c_float),         # [1001] distance from start
...
class CBEAM_TSN(Structure):        # 104/LC:Z!  thermal eigen stress per material
   _fields_ = [
         ('m_nr', c_int),          #        element number
...
class CDSLN_TSN(Structure):        # 124/LC:Z!  thermal eigen stress per material
   _fields_ = [
         ('m_nr', c_int),          #        element number
         ('m_mnrx', c_int),        #        10000+materialnumber
         ('m_x', c_float),         # [1001] distance from start
...
class CBSCT_TSN(Structure):        # 144/LC:Z!  thermal eigen stress per material
   _fields_ = [
         ('m_nr', c_int),          #        element number
         ('m_mnrx', c_int),        #        10000+materialnumber
         ('m_x', c_float),         # [1001] distance from start
...
class CBEAM_CRP(Structure):        # 104/LC:Z!  forces on sectional parts per material
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_mnr', c_int),         #        materialnumber
         ('m_x', c_float),         # [1001] distance from start
...
class CDSLN_CRP(Structure):        # 124/LC:Z!  forces on sectional parts per material
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_mnr', c_int),         #        materialnumber
         ('m_x', c_float),         # [1001] distance from start
...
class CBSCT_CRP(Structure):        # 144/LC:Z!  forces on sectional parts per material
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_mnr', c_int),         #        materialnumber
         ('m_x', c_float),         # [1001] distance from start
...
class CBEAM_CRT(Structure):        # 104/LC:Z!  changes of tendon force
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
...
class CDSLN_CRT(Structure):        # 124/LC:Z!  changes of tendon force
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_nrs', c_int),         #        neg. tendonnumber
         ('m_x', c_float),         # [1001] distance from start
...
class CBSCT_CRT(Structure):        # 144/LC:Z!  changes of tendon force
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_nrs', c_int),         #        neg. tendonnumber
         ('m_x', c_float),         # [1001] distance from start
...
class CBEAM_STC(Structure):        # 105/LC:0  Maximum stresses in cross-section of beams
   _fields_ = [
         ...
         ('m_sigc', c_float),      # [1092] minimum stress
         ('m_sigt', c_float),      # [1092] maximum stress
...
class CBEAM_STR_SHEA(Structure):   #        factors of warp and shear per construction stage (MAX_SECT_CS+1
   _fields_ = [
         ('m_esw', c_float),
         ('m_vyf', c_float),       # [1102] 
         ('m_vzf', c_float),       # [1102] 
...
cbeam_str_shea = CBEAM_STR_SHEA()

class CBEAM_STR(Structure):        # 105/LC:Z!  Stresses in cross-section of beams
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_mnr', c_int),         #        materialnumber
         ('m_x', c_float),         # [1001] distance from start
         ...
         ('m_shea',  CBEAM_STR_SHEA * 33)  #        factors of warp and shear per construction stage (MAX_SECT_CS+1
      ]
cbeam_str = CBEAM_STR()

class CDSLN_STR_SHEA(Structure):   #        factors of warp and shear per construction stage (MAX_SECT_CS+1
   _fields_ = [
         ('m_esw', c_float),
         ('m_vyf', c_float),       # [1102] 
         ('m_vzf', c_float),       # [1102] 
...
cdsln_str_shea = CDSLN_STR_SHEA()

class CDSLN_STR(Structure):        # 125/LC:Z!  Stresses in cross-section of design elements
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_mnr', c_int),         #        materialnumber
         ('m_x', c_float),         # [1001] distance from start
         ...
         ('m_yc', c_float),        # [1011] location of center of partial section
         ('m_zc', c_float),        # [1011] location of center of partial section
         ('m_shea',  CDSLN_STR_SHEA * 33)  #        factors of warp and shear per construction stage (MAX_SECT_CS+1
      ]
cdsln_str = CDSLN_STR()

class CBSCT_STR_SHEA(Structure):   #        factors of warp and shear per construction stage (MAX_SECT_CS+1
   _fields_ = [
         ('m_esw', c_float),
         ('m_vyf', c_float),       # [1102] 
         ('m_vzf', c_float),       # [1102] 
...
cbsct_str_shea = CBSCT_STR_SHEA()

class CBSCT_STR(Structure):        # 145/LC:Z!  Stresses in cross-section of external sections
   _fields_ = [
         ('m_nr', c_int),          #        beamnumber
         ('m_mnr', c_int),         #        materialnumber
         ('m_x', c_float),         # [1001] distance from start
         ...
         ('m_yc', c_float),        # [1011] location of center of partial section
         ('m_zc', c_float),        # [1011] location of center of partial section
         ('m_shea',  CBSCT_STR_SHEA * 33)  #        factors of warp and shear per construction stage (MAX_SECT_CS+1
      ]
cbsct_str = CBSCT_STR()

class CBEAM_CST(Structure):        # 1105/LC:Z!  Composite stresses in cross-section of beams
   _fields_ = [
         ('m_nr', c_int),          #        beam/dsln id
         ('m_mnr', c_int),         #        material id
         ('m_wrklaw', c_int),      #        worklaw (see 105: rctrl WORKLAW_...)
         ('m_elmid', c_int),       #        cross-section element id     (parent element in case of reinfor
         ('m_subid', c_int),       #        cross-section element sub-id (parent element in case of reinfor
         ('m_cs', c_int),          #        construction stage
         ('m_id1', c_int),         #        free
         ('m_id2', c_int),         #        free
         ('m_id3', c_int),         #        free
         ('m_x', c_float),         # [1001] distance from start
         ('m_rho_s', c_float),     # [1299] reinforcement degree (only concrete elements)
         ('m_beta_t', c_float),    #        tension stiffening factor (only reinforcements)  (see 3)
         ('m_be1', c_float),       # [1011] effective width at start            (see 4)
         ('m_be2', c_float),       # [1011] effective width at end              (see 4)
         ('m_es0', c_float),       #        strain at origin of cross-section   (see 1)
         ('m_esy', c_float),       #        strain gradient in y-direction      (see 1)
         ('m_esz', c_float),       #        strain gradient in z-direction      (see 1)
         ('m_ew0a', c_float),      #        strain from warping at start point  (see 1)
         ('m_ew0b', c_float),      #        strain from warping at end   point  (see 1)
         ('m_taua', c_float),      #        shear stress factor a               (see 2)
         ('m_taub', c_float),      #        shear stress factor b               (see 2)
         ('m_tauc', c_float),      #        shear stress factor c               (see 2)
         ('m_taulin', c_float)     #        shear stress in perpendicular dir   (see 2)
      ]
cbeam_cst = CBEAM_CST()

class CDSLN_CST(Structure):        # 1125/LC:Z!  Composite stresses in cross-section of design elements
   _fields_ = [
         ('m_nr', c_int),          #        beam/dsln id
         ('m_mnr', c_int),         #        material id
         ('m_wrklaw', c_int),      #        worklaw (see 105: rctrl WORKLAW_...)
...
class CBEAM_DE0(Structure):        # 107/LC:0  max. values of Ultimate/Plastic Design results
   _fields_ = [
         ...
         ('m_c2t', c_float),       # [1299] utilisation level (21) plate slenderness (c/t) (obsolete)
...
class CBEAM_UC0(Structure):        # 1107/LC:0  Maximum utilisations in cross-section of beams
   _fields_ = [
         ('m_nr', c_int),          #        id = 0 := maxima
         ('m_id1', c_int),         #        free
         ('m_id2', c_int),         #        free
         ('m_class', c_int),       #        Section class, resulting (== cls_pl, if pl check is fulfilled, 
         ('m_cls_pl', c_int),      #        Section class, plastic (if plastic class>2 : cls_pl=-2)
         ('m_cls_el', c_int),      #        Section class, elastic
         ('m_check', c_int),       #        Bitpattern: 1: plastic, 2: elastic check fulfilled
         ('m_x', c_float),         # [1001] distance from start
         ('m_val1', c_float),      #        free
         ('m_val2', c_float),      #        free
         ('m_eta_my', c_float),    #        Plastic utilisation: My/Mypl (neg. if not applicable, i.e., pla
         ('m_eta_vz', c_float),    #        Plastic utilisation: Vz/Vzpl
         ('m_eta_el', c_float)     #        Elastic utilisation: von Mises, with buckling reduction if elas
      ]
cbeam_uc0 = CBEAM_UC0()

class CDSLN_UC0(Structure):        # 1127/LC:0  Maximum utilisations in cross-section of design elements
   _fields_ = [
         ('m_nr', c_int),          #        id = 0 := maxima
         ('m_id1', c_int),         #        free
         ('m_id2', c_int),         #        free
...
class CBEAM_UCD(Structure):        # 1107/LC:Z!  Composite design utilisations in cross-section of beams
   _fields_ = [
         ('m_nr', c_int),          #        beam/dsln id
         ('m_id1', c_int),         #        free
         ('m_id2', c_int),         #        free
         ...
         ('m_cls_pl', c_int),      #        Section class, plastic (if plastic class>2 : cls_pl=-2)
         ('m_cls_el', c_int),      #        Section class, elastic
         ('m_check', c_int),       #        Check bitpattern
         ('m_x', c_float),         # [1001] distance from start
         ('m_val1', c_float),      #        free
         ('m_val2', c_float),      #        free
         ('m_eta_my', c_float),    #        Plastic utilisation: My/Mypl (neg. if not applicable, i.e., pla
...
class CDSLN_UCD(Structure):        # 1127/LC:Z!  Composite design utilisations in cross-section of design elemen
   _fields_ = [
         ('m_nr', c_int),          #        beam/dsln id
         ('m_id1', c_int),         #        free
         ('m_id2', c_int),         #        free
...
class CBEAM_HR0(Structure):        # 111/LC:0  Maximum of Implicit Hinge Reactions
   _fields_ = [
         ...
         ('m_pidx', c_float * 5)   #        extremal values of performance indices by category
      ]
cbeam_hr0 = CBEAM_HR0()
...
class CBEAM_HRC(Structure):        # 111/LC:+  Implicit Hinge Reactions
   _fields_ = [
         ...
         ('m_pidx', c_float * 5)   #        performance indices by category, cf. Rec: 16/0:0   PERF_CAT
      ]
cbeam_hrc = CBEAM_HRC()
...
class CBEAM_MP0(Structure):        # 115/LC:0  Extremal values of cross-sectional material point reactions
   _fields_ = [
         ...
         ('m_f0', c_float),        #        utilisation level
...
class CSPRI_WL0(Structure):        # 173/NR:0  (Nonlinear) Material for Link elements, Spring elements and Bea
   _fields_ = [
         ...
         ('m_prog', c_int),        #        Generating source, encoded name of generating program
...
class CSPRI_WL(Structure):         # 173/NR:+  force-displacement / moment-rotation work law
   _fields_ = [
         ('m_id', c_int),          #        Identification
         ('m_ntyp', c_int),        #        Type of law
         ('m_plvl', c_float),      # [1101] Normal force reference level
         ('m_eur', c_float),       #        Elastic unloading/reloading stiffness [kN/m],[kNm/rad]
         ('m_crac', c_float),      # [1089] rupture stress (cracking), [1156] for SPRI_WL_ID_CM
         ('m_gapp', c_float),      # [1003] intial slip in positive direction (magnitude), [1004] for SPRI_
         ('m_gapn', c_float),      # [1003] intial slip in negative direction (magnitude), [1004] for SPRI_
         ('m_fric', c_float),      #        friction coefficient
         ('m_coh', c_float),       # [1089] cohesion
         ('m_par', c_float * 3),   #        further reserved material parameters
         ('m_arb', c_float * 20 * 5)  #        Function values
      ]
cspri_wl = CSPRI_WL()

class CSPRI_WLP(Structure):        # 173/NR:9??  Optional performance limits for force-displacement / moment-rot
   _fields_ = [
         ('m_id', c_int),          #        Identification
         ('m_cat', c_int),         #        category of set of performance points, 4-character string, cf. 
         ('m_val1', c_float),      #        unused
         ('m_val2', c_float),      #        unused
         ('m_par', c_float * 8),   #        unused
         ('m_plims', c_float * 20 * 5)  #        Performance limits
      ]
cspri_wlp = CSPRI_WLP()
...
class CS_MATRIX(Structure):        # 175/00  General systemmatrices
   _fields_ = [
         ...
         ('m_ndn', c_int),         #        max SYS_DOF_DIR used by matrix (cf. "storage scheme" below)
...
class CQUAD(Structure):            # 200/00  QuadElements
   _fields_ = [
         ...
         ('m_mat', c_int),         #        materialnumber
         ('m_mrf', c_int),         #        material reinforcement
         ('m_nra', c_int),         #        type of element
         ('m_det', c_float * 3),   #        Parameter of Jacobi Determinant
         ('m_thick', c_float * 5), # [1010] element thickness
         ('m_cb', c_float),        #        bedding factor
...
class CQUAD_NOH(Structure):        # 200/1:0  header of unified properties of Quad-nodes
   _fields_ = [
         ('m_id1', c_int),
         ('m_id2', c_int),
         ('m_nkrec', c_int),
         ...
         ('m_nquad', c_int),
         ('m_mquad', c_int),
         ('m_d_all', c_float)      # [1010] uniform thickness of all Quads
      ]
cquad_noh = CQUAD_NOH()

class CQUAD_NGH(Structure):        # 211/0:0  header of grouped properties of Quad-nodes
   _fields_ = [
         ('m_id1', c_int),
         ('m_id2', c_int),
         ('m_nkrec', c_int),
...
class CQUAD_NOM(Structure):        # 200/1:-  unified Quad-node mapping to elements
   _fields_ = [
...
class CQUAD_NGM(Structure):        # 211/0:-  grouped Quad-node mapping to elements
   _fields_ = [
         ('m_ngt', c_int),         #        groupselector
         ('m_nr', c_int),          #        nodenumber
         ('m_iel', c_int * 9999)   #        connecting elements
...
class CQUAD_NOD(Structure):        # 200/1:Z+  unified properties of Quad-nodes
   _fields_ = [
...
class CQUAD_NGD(Structure):        # 211/0:Z+  grouped properties of Quad-nodes
   _fields_ = [
         ('m_ng', c_int),          #        groupnumber
         ('m_nr', c_int),          #        nodenumber
         ('m_d', c_float),         # [1010] minimum thickness of plate on node
...
class CQUAD_RNC(Structure):        # 215/LC:0  nonlinear results of Quad-element
   _fields_ = [
         ...
         ('m_epsr', c_float)       # [1081] Maximum reinforcement strain in the crack
      ]
cquad_rnc = CQUAD_RNC()
...
class CQUAD_RNG(Structure):        # 215/LC:-  nonlinear results of Quad-Gauss points
   _fields_ = [
...
class CQUAD_DSC(Structure):        # 250/DC:0  maximum design stresses in Quad-elements
   _fields_ = [
         ('m_nr', c_int),          #        ident 0
...
class CQUAD_NDC(Structure):        # 251/DC:0  maximum design stresses in Quad-nodes
   _fields_ = [
         ('m_ng', c_int),          #        ident 0
         ('m_nr', c_int),          #        ident 0
         ('m_smio', c_float),      # [1092] minimum upside main stress
...
class CQUAD_RIC(Structure):        # 260/DC:0  maximum reinforcement in Quad-elements
   _fields_ = [
         ...
         ('m_utilc', c_float),     #        max. utilisation fatigue check concrete
...
class CQUAD_NRP(Structure):        # 262/DC:+  punching reinforcement in nodes
   _fields_ = [
         ...
         ('m_redo', c_float),      # [1299] reduction factor for openings/free edges
         ('m_v', c_float),         # [1151] reduced punching force
         ('m_asl', c_float),       # [1021] required longitudinal reinforcement
         ('m_asv', c_float * 8),   # [1020] shear reinforcement within perimeter 1:8
         ...
         ('m_vmax', c_float)       # [1092] shear stress at reduced critical 1st perimeter
      ]
cquad_nrp = CQUAD_NRP()
...
class CBRIC_NOR(Structure):        # 325/LC  Bric nonlinear reinforcement results
   _fields_ = [
         ...
         ('m_oaf', c_float),       #        Euler Angle II
         ('m_sigs', c_float * 3),  # [1092] steel stress 1-3 Maximum of gauss points
         ('m_sigb', c_float),      # [1092] maximum concrete pressure Maximum of gauss points
         ('m_shzmax', c_float),    #        maximum skew principal tensile stress (SLS uncracked) Maximum o
         ('m_wkr', c_float * 3),   # [1026] crack width 1-3 in direcition of reinforcement
         ...
         ('m_sigsl0', c_float * 6),# [1092] 3 steel stress SIGS and 3 linear steel stress SLIN gauss point 
